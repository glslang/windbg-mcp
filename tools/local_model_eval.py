...
def one_arm_per_log(plan, log_path):
    """Refuse a plan that asks for a reasoning arm the log it writes to does not already hold.
    ...
    *run*, like the server build and the weights, and lives in `identity()` beside them. Putting
    ...
    # write the second arm, and it is the case a plan-only guard cannot see: `already_done` keys
    # without `think`, so every cell reads as finished, the run does nothing at all, and the log
    # still holds the arm nobody asked for. Refused here rather than reported afterwards - a run
    # that measures zero tasks and exits 0 looks exactly like a resume with nothing left to do.
    for cell_id, arms in reasoning_arms(log_path).items():
        backend, model, num_ctx, surface = cell_id
        if backend != "ollama":
            continue
        asked = seen.get((model, num_ctx, surface))
        if asked is None:
            continue
        held = arms - {"on" if asked else "off"}
        if held:
            raise SystemExit(
...
def print_compare(order, rows, identities, surfaces, old_path, new_path):
    ...
    moved = [name for name in ("suite", "server", "reasoning")
             if old_ident[name] != new_ident[name]]
    # **Compared only where both runs have an answer, because absence is composition and not
    # movement.** Arm B of a reasoning A/B has no Claude rows at all - their reasoning belongs to
    # a client this bench does not drive - so `harness` and the `opus`/`sonnet` weights are
    # *missing* from it rather than different in it. Naming those as moved put "the weights
    # changed" above a table whose four shared models carried byte-identical digests, which is
    # the one line a reader takes at face value. A model dropped between two runs still shows: as
    # `(old)` rows in the table below, which is where a composition difference belongs.
    if old_ident["harness"] and new_ident["harness"] and old_ident["harness"] != new_ident["harness"]:
        moved.append("harness")
    shared = set(old_ident["weights"]) & set(new_ident["weights"])
    if any(old_ident["weights"][model] != new_ident["weights"][model] for model in shared):
...
def suite_for(log_path, fallback):
    """The suite a log says it ran against, or `fallback` for logs written before it said.

    **A series spans a rewording, so one suite for every log is the wrong shape.** The rule this
    bench already had - *grade a published log against the suite it ran on* - was enforced by
    hand: `docs/eval-runs.json` was built by two invocations and merged, which meant the file
    could not be regenerated by the command the page documents, and running that command would
    have re-graded the newer runs against the older wording and relabelled every row as v1.

    The log is asked rather than the plan beside it, because the log is what survives: each
    record carries the suite's file name, so a log is self-describing even with no plan on disk.
...
def series(log_paths, tasks_file, out_path):
    ...
    for log_path in log_paths:
        ...
        rows.append({
            ...
            "identity": ident,
            "graded_against": os.path.basename(suite),
