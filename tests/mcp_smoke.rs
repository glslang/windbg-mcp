...
/// exactly that. The payload went 225,727 -> 231,136, a difference of 5,409: `ioctl_map` is
/// 5,337 B of wire, its `TOOL_NOTES` cross-reference adds 71 B to `driver_object`, and the
/// remaining byte is the array's own comma. Nothing else moved. (The tool was 5,245 B when this
/// paragraph was first written and 92 B of `outputSchema` were added by review, which is the
/// ordinary way this figure moves: two fields on a case.) It shares `CodeLocation`, `ImageRef` and `WalkHalt`
