...
[![Latest release](https://img.shields.io/badge/release-v0.8.1-blue)](https://github.com/glslang/windbg-mcp/releases/latest)
...
| `modules` | `modules[]` with `start`/`end`, `size` and a typed `symbols` state (`deferred` is *not* `none`); `unloaded[]` for the images that have since unloaded (listed by image name, since an unloaded module has none of its own); `loaded` (how many the target has in total) and the `filter` a narrowed listing was matched by — an ASCII name plus `*` and `?` is the whole grammar it honours, and anything else (WinDbg's `[fd]`/`#`/`+`/`\`, a space, which `lm m` reads as the start of its own options, or a character whose case the two sides may fold differently) is refused rather than applied to the text alone |
