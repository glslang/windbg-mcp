...
|------|----------|---------------|
| **Openable** | Can this token open the device at all? | `device_security` → the DACL as principals and access masks, with `secure_open` beside it. `security_absent` is the exception and may be unreachable — every device object on a measured Windows Server 26100 guest carries one, unnamed devices included — and where it *is* answered the tool reports the gate rather than resolving it: the claim that the holding directory is checked instead is not one this has verified, so inspect that directory's DACL yourself. On a dump, where there is no object namespace: `device_object` for the `SecurityDescriptor` pointer, then `!sd <ptr>` where that extension is present |
