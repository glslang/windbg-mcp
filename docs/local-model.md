...
| | bytes | ≈tokens |
|---|---|---|
| The tool surface, paid once per conversation | 79,825 (56 tools) | ~20k |
| — the same surface as `--tools session,inspect,crash` | 26,305 (20 tools) | ~6.5k |
| — as `--tools crash` | 18,026 (13 tools) | ~4.5k |
...
- **The tool-surface profile is `--tools`** (2026-08-22). Start the listener with
  ...
  68,893 — `--tools crash` is 13 and 18,026 B, which is the difference between "roughly twice an 8k
