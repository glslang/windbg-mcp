...
- `device_security` reports creation call sites, recovered SDDL defaults, class GUIDs, device characteristics, and symbolic-link literals. Effective access and namespace visibility remain dynamic findings because installation/runtime configuration can override defaults.
  **The native tool of that name answers the dynamic half instead, so the two are complements rather than one implementation of the other.** `device_security` in `windbg-mcp` walks the live kernel's object namespace: the descriptor it reports is the one on the device object now, the characteristics are the words `IoCreateDevice` actually wrote, and the symbolic links are the ones in `\GLOBAL??` at this moment -- which is exactly the "effective access and namespace visibility" this row leaves open. A companion recovering the SDDL default from the image says what the driver asked for; the native one says what it got.
...
- Implement `driver_surface` as a composite over the same four core functions. Preserve per-section success, partial, unavailable, and error outcomes, including prerequisites and traversal bounds. A failed dispatch recovery must not discard import or security evidence.
  **The native tool implements the composite and the per-section outcomes, and not the traversal.** `driver_surface` in `windbg-mcp` carries `ok`/`partial`/`unavailable`/`error` with a note per section, and its IOCTL and hazard sections are `ioctl_map`'s and `driver_hazards`' results whole -- so a failed dispatch recovery genuinely leaves the import and security evidence standing. What it does **not** do is the bounded dispatch-to-sink traversal the row above enables from it: the cases and the sinks are reported side by side with nothing joining them, so "which control code reaches `MmMapIoSpace`" is still a `reachable_from_dispatch` call per pair. `FOLLOWUPS.md` item 71 is that gap, with the bounds this plan fixes.

The native side implements this shape: `ioctl_map` in `windbg-mcp` emits exactly these fields (plus `recovered`, `at` and an `evidence[]` of length checks), recovered from the debugger's own disassembly of the dispatch routine. A companion answering the same shape is joined to it by the image's **identity** and RVA -- the GUID, age and timestamp `modules` reports -- rather than by a module name, which repeats across builds and would attach one build's cases to another's.
