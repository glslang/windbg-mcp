...
- `reachable_from_dispatch` is **x86, x64 and ARM64**, and refuses anything else by name. It
  ...
  instruction *is* rather than where it goes. Measured on `docs/samples/082126-7015-01.dmp`
...
- `driver_hazards` is **evidence, not a verdict**, and each of its three limits is a way to reach a
  ...
  **absent import excludes nothing**, because a driver can resolve an export at run time through
  ...
  anything else by name, **ARM64 included** -- the same operand gate `ioctl_map` has and not the
  reachability walk's flow gate. `privileged` is the decoder's own answer about an instruction, and
