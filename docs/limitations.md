...
- **A device that carries no descriptor is a branch nothing measured has reached, and its advice is unverified.** Where the field is empty this tool reports that and tells the reader the object manager checks the directory holding the device instead -- it does **not** read that directory, and *the claim that the directory is what gets checked has never been measured either*. Nor has the branch been reached: on a Windows Server 26100 guest every one of the **231** device objects on every driver chain carries a descriptor, including the **66** that have no name and so cannot be looked up by path at all. So `security_absent` is a branch whose sentence is owed a measurement rather than a case to plan around. `FOLLOWUPS.md` item 68 is what settling it would take.
