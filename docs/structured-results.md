...
| `set_breakpoint` | the ids this call `added`, and every breakpoint now set — a successful `bp` prints nothing at all. Two fields say the answer is less than it looks, and both are successes rather than errors because `bp` is not idempotent and an error is the shape a caller retries: `listed` false means the follow-up inspection failed *after* the breakpoint was set, so which one is new is unknown; `cut_short` means the `bp` did not finish — the watchdog broke it in while it resolved the expression, or somebody called `interrupt` — so `added` being empty is "not set yet" rather than "matched nothing". One field for both causes, unlike a stop's `interrupted`/`timed_out` pair, because here the next move does not depend on which: the listing is read either way and is what tells the two apart. `added` non-empty under `cut_short` is a breakpoint that landed before the break and must not be re-requested; empty is *not* evidence the expression is unset, because a `bp` at an address that already carries a breakpoint adds no id either — read the listing. (Repeating a `bp` is a no-op once the expression resolves, since a resolved breakpoint is keyed by address; a **deferred** one duplicates, which is why the warnings are worth their words) |
