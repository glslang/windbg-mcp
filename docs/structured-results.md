...
| `break_in` | `requested`, whether that run is going to stop — a break raised, one already lodged, or a run barred from starting because it had not begun. `false` means it had already finished, which is the ordinary race and not a failure, while a break that could not be *delivered* is an error rather than a `false`. Plus the debugger's own `detail` |
