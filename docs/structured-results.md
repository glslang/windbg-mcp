...
| `ioctl_map` | `cases[]` as `{code, device_type, function, method, required_access, dispatch_rva, case_rva, in_size, out_size, recovered, at, evidence[]}` -- the control codes the dispatch routine accepts, each decoded from its own bits, with `at` the `{address, module, rva}` location of the compare or jump that recognises it and `case_rva` the block it routes to. The field names are the **shared IOCTL-case shape** a Binary Ninja or Ghidra companion answers in (`docs/binja-windbg-mcp-plan.md`), so the same driver recovered on either side is the same record. `recovered` says how: `compare` for a compare against the code, `jump_table` for an entry in a switch table that was resolved -- and a table is followed only when its bounds check **and** its base were both recovered, never at a guessed length, never across a `call` between the load and the jump (a callee returns over the volatile registers, so the jump goes wherever it returned), never from a load that is not a `DWORD` (the slots are decoded four bytes at a time, so anything else is read as halves of entries nobody computed), and never when the index is the control code **shifted** -- several codes reach each slot of such a table and the walk cannot say which of them is taken, so the jump is in `unresolved[]` instead. A case per **site**: one code compared in two places is two records. `in_size` and `out_size` are proven exact lengths or absent, and never a floor -- a `cmp [sp+10h],20h` / `jb` proves a minimum and proves nothing about the size, so it is in `evidence[]` as a `{field, value, exact, at}` check instead. `tables[]` records each switch table that was followed, where `followed` is how many of its `entries` became cases -- the difference is the slots that go to the switch's default, which a dense table has one of for every index it has no case for. `unresolved[]` lists the indirect **jumps** that were not followed, which is what stops a short list reading as a complete one: with entries there the map is a lower bound on what the driver accepts. A call through a pointer is not one of them -- a driver reaches its imports that way, so every dispatch routine has many and none decides on a control code. Each case carries `accepted`, which is what the compare and the branch do **not** say: `cmp code,N` / `je invalid_request` is the same shape as `je handler`, so a list reporting both as accepted sends a reader to test a code the driver refuses. It is `false` for a landing block that sets an NTSTATUS error and returns -- the ordinary rejection stores that status into `Irp->IoStatus.Status` and calls a completion routine on its way out, so a call alone is never read as acceptance; it is that field off a register the walk watched the IRP reach and not any store, since a constant put in a stack local is not a refusal -- `true` for a block that reaches a routine with no rejection visible in it, and absent where neither is. `in_size` and `out_size` need the same evidence: an exact size is a check whose *other* edge fails, since `cmp length,20h` / `jne handler` requires nothing, and it is a check at the field's own width, since `cmp cx,20h` admits every length whose low sixteen bits are 32. A case whose `recovered` is `jump_table` carries neither size and no `evidence[]`: an indirect jump contributes no edge to the graph the walk settles facts over, so what is on the way into its landing block is whatever a *direct* predecessor left -- and a landing shared with a compare would otherwise publish that compare's length check as the table entry's own requirement. Each case also carries `proved`, which says whether the value it tested was traced from the IRP (`[[Irp+0xb8]+0x18]`) or taken from a bare displacement, and `code_proved` is every case together rather than any one load -- a routine that compares some other structure's `+0x18` field before reading the real control code has one of each, and the guessed one is exactly the case a reader needs warning about. The offsets themselves follow the target's bitness: `+0xb8`/`+0x18` on x64, `+0x60`/`+0x0c` on x86, where both arguments arrive on the stack so nothing is proved. `blind` counts instructions that could not be read or decoded, each a place a compare may be. What an empty `cases` means turns on `code_proved` -- one says the routine compares the code against nothing this could follow, the other that these compares may be about another structure entirely. `unproved` counts the cases whose value was not traced to the IRP, exact past the cap as the count is -- taken from the cases that were **kept** it reads zero under a `code_proved: false` that fired because the dropped one was the unproved one. `stopped` and `cap_hit` say why a list ended early -- `cap_hit` for a bound in the tool, including a switch whose bounds check admits more entries than one table may carry, and `stopped` for a clock that ran out, which ends the case list where it ended the work rather than listing codes with no handler, acceptance or sizes read for them; `case_count` stays exact past its cap, while `tables[]` and `unresolved[]` are bounded without a count, a truncated one of those supporting the same reading as a full one. `unsettled` is the other way an answer is short and means something else entirely: what each block knows never stopped changing inside the walk's sweep budget, so every case and table was **discarded** rather than reported -- a routine no answer was settled about, which an empty `cases` alone cannot tell you from one with no control codes |
