...
| `break_in` | `requested`, whether a break is now lodged for that run — `false` where it had already stopped, which is the ordinary race and not a failure, while a break that could not be *delivered* is an error rather than a `false` — and the debugger's own `detail` |
