...
| `go`, `step_over`, `step_into`, `step_back`, `step_over_back`, `reverse_go` | `stopped_at`, and the `thread` it belongs to plus, on a kernel target, the `processor` — absent where no processor number applies, which every user-mode target is. Plus `interrupted` and `timed_out` — two reasons the position is real and is *not* a stop the target reached: somebody called `interrupt`, or the wait ran out and the debugger broke the target in — and `target_gone`, the ending where there is no position at all because the target ran to completion |
| `continue_async` | `execution` (the handle), `command`, `running` (false only where the command completed without the target ever moving, in which case the stop is already recorded against the handle) and `breaks_in_ms`, when the debugger will break the target in itself |
...
| `break_in` | `requested`, whether the break reached the engine — `false` where the run had already stopped, which is the ordinary race and not a failure — and the debugger's own `detail` |
