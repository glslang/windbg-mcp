...
| **Live hypervisor** | `--ignored` + `WINDBG_MCP_SMOKE_HYPERVISOR_PROFILE` | a disposable Microsoft hypervisor target, reached by its own configured profile | hypervisor identity, registers, memory, disassembly, one step, breakpoint set/clear, and explicit detach; [runbook and limits](hypervisor-debugging.md#validation) |
