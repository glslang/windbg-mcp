...
- **A kernel attach left a break-in owing, and the teardown paid it with the target's only continue.** `DEBUG_ENGOPT_INITIAL_BREAK` leaves a pending host break-in; dbgscope's `absorb_initial_break_artifact` consumes exactly one, which is right for NT and one short on a Microsoft hypervisor. Nothing in the attach's result shows the leftover -- the session looks ordinary -- and `end_session` then spends it: `qd` sends one `DbgKdContinue`, the pending break-in takes it, and the target stops again with **no debugger attached**. The guest is frozen while this server reports `released: true` and `target_left_running: true`, which is [`FOLLOWUPS.md` item 93](FOLLOWUPS.md)'s whole shape. `worker::drain_pending_break_ins` now spends them first, resuming with a 500 ms bound until **two consecutive** resumes run free -- one free run is not evidence, since a break-in that is merely slow to arrive reads the same way. The session's breakpoints come off the target before the first of those resumes, because a resume that reaches one cannot be told from the break-in it is hunting: an armed breakpoint would spend every attempt, leave the leftover owing, and be counted in the log as break-ins consumed. A target whose breakpoints will **not** come off is drained anyway and said to be unattributable, rather than skipped: the teardown's own clearing is a fresh attempt that may succeed where this one did not -- it removes by index where this removes by id -- and skipping would then send `qd` with the break-in still owed, which is a certain freeze bought to avoid a possible misread. What a failed clearing costs is the reading, not the resume, because the rule that a kernel with an unremovable `int 3` is left halted rather than run is about the resume that *hands the target back*: `qd` sets it going and detaches, while a drain resume is bounded with the debugger still attached. Measured on a one-vCPU lab, plain attach and detach: frozen **2 of 2** before, healthy **5 of 5** after, each confirmed by WinRM showing the same boot and advancing uptime. NT is unaffected, checked by two pool-walk cycles against a four-processor guest. Two readings pinned the cause: stepping past the hypervisor's own `int 3` before detaching does *not* help, so it is not where the instruction pointer sits; and the first `go` after a completed attach returns at once with DbgEng's CTRL+BREAK banner while every later one runs to its bound, which is the leftover showing itself. The drain runs on **both** teardown paths -- an `end_session`, and the release a worker performs for itself when the supervisor disappears without saying goodbye, which is the path a crash or a Ctrl+C takes and the one with nobody left to ask again -- and that path's own exit grace is extended by the drain's worst case, since a drain cut off by the grace hands the break-in back to `qd` and leaves exactly the halted target the grace exists to prevent. The teardown is **sealed against interrupts** as it begins, the way a `debug_batch`'s rollback already was: a resume cut short by a host's interrupt is indistinguishable from one that found nothing pending, so a client interrupting its own `end_session` could otherwise end the drain early and hand the leftover straight back to `qd`. A caller that asks is told so instead of taking a break, in a refusal written for a teardown rather than the rollback's -- whose advice is to run `end_session`, which here is what is already running.

- **The hypervisor tier's broader test called a tool that did not exist.** It asked for `breakpoints`, passed `address` where `set_breakpoint` takes `expression` -- refused outright, since those arguments deny unknown fields -- and attached on the default path, which has nothing to break into on a hypervisor that is running. All three are why the 2026-09-20 demonstrations were driven by a temporary uncommitted harness reading `bl` text, and why the one thing 0.19.0 led with had no committed regression. The test now uses the two tools above, takes its attach shape from `WINDBG_MCP_SMOKE_HYPERVISOR_BREAK_ON_CONNECT`, and carries the breakpoint-hit half -- a return address read off the stopped processor's stack, checked to be inside the `hv` image this session attached to, run to, and asserted `hit` with an empty inventory afterwards -- behind `WINDBG_MCP_SMOKE_HYPERVISOR_BREAKPOINT_HIT`. That gate is opt-in because [`FOLLOWUPS.md` item 93](FOLLOWUPS.md) is open: on the four-processor lab that sequence left the guest frozen until a separately authorised connection released further stops. `examples/hypervisor_detach_regression.ps1` gained `-Session`, `-BreakpointHit`, and a refusal to pass the latter to a guest reporting more than one logical processor -- read over WinRM before **every** cycle, since a VM's processor count changes with a restart.
